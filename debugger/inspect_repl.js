...
list, ls              Print the source around the current line where execution
                      is currently paused
...
function createRepl(inspector) {
  ...
  class CallFrame {
    ...
    relativeSourcePathWithLineAndColumnNumbers() {
      const { scriptId, lineNumber, columnNumber } = this.location
  ...
  class Backtrace extends Array {
    [customInspectSymbol]() {
      return ArrayPrototypeJoin(
        // TODO optional filtered backtraces
        ArrayPrototypeMap(this, (callFrame, idx) => {
          ...
          const frameLocation = callFrame.relativeSourcePathWithLineAndColumnNumbers()

          const isCurrentlySelected = selectedFrame === callFrame;
          const selectionFormatting = isCurrentlySelected ? '* ' : '  ';

          return `${selectionFormatting}#${idx} ${name} ${frameLocation}`;
        }), '\n');
    }

    // wraps all raw callFrames in a CallFrame class for improved debugging ergonomics
  ...
  async function evalInCurrentContext(code) {
    // TODO should be extracted out into a a helper function
  ...
  function relativeFilePath() {
    ...
    return selectedFrame.relativeSourcePathWithLineAndColumnNumbers()
  }
  ...
  Debugger.on('paused', ({ callFrames, reason, data, asyncStackTrace }) => {
    ...
    // TODO should refactor to use line detection
  ...
  function initializeContext(context) {
    ...
    copyOwnProperties(context, {
      ...
      get up() {
        if (currentBacktrace) {
          const idx = ArrayPrototypeIndexOf(currentBacktrace, selectedFrame);

          if (idx + 1 < currentBacktrace.length) {
            selectedFrame = currentBacktrace[idx + 1];
            return currentBacktrace
          } else {
            print('Already at the top of the stack');
          }
        } else {
      ...
      frame(index) {
        // jump to the frame at the given index
        if (currentBacktrace) {
          if (index < 0 || index >= currentBacktrace.length) {
            print(`Invalid frame index ${index}`);
            return;
          }

          selectedFrame = currentBacktrace[index];
          return currentBacktrace
        }
      },
      ...
      get locals() {
        return evalInCurrentContext('.scope');
      },
